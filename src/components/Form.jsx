...
/**
 * [FILE SIZE]
 * This file is too long (700 lines). Even with languages like PHP,
 * developers will often have a rule to never go above 300 lines of code.
 *
 * With React, I would argue that this should be even less.
 * One of the benefits of React, is the ability to atomize your components.
 *
 * I.e. having small (atomized) components that can be used across your app.
 * E.g. having a components folder, with a button that you can reuse.
 */
      ...
      <StyledInput>
        <label htmlFor="qty">
          <input
            ...
            onChange={handleChange}
          />
          {errors[`qty${index}`] && <span>{errors[`qty${index}`]}</span>}
        </label>
      </StyledInput>
      <StyledInput>
        <label htmlFor="price">
          <input
            ...
            placeholder="0.00"
          />
          {errors[`price${index}`] && <span>{errors[`price${index}`]}</span>}
        </label>
      </StyledInput>
      <StyledInput>
...
export default function Form() {
  ...
  const [items, setItems] = useState(
    // the use of conditionals in setting your initial form state is a symptom of managing your own form state. Try refactoring and using a form library
  ...
  const validateField = (name, value) => {
    // also a symptom of not using a form library
    let error = "";
  ...
  const validateForm = () => {
    ...
    items.forEach((item, index) => {
      ...
      if (!item.qty || item.qty <= 0) currentErrors[`qty${index}`] = "Required";
  ...
  const navigate = useNavigate(); // it's good practice to have all your hooks in the first lines of your component code. If you had a conditional before this line, e.g. (if (!something) render null;) it would crash the app.
      ...
      <StyledFormHeader>
        ...
        <div className="inputsWrapper">
          <StyledInput>
            ...
            />
            {errors.invoiceNumber && <span>{errors.invoiceNumber}</span>}
          </StyledInput>
          <StyledInput>
            ...
            />
            {errors.purchaseOrder && <span>{errors.purchaseOrder}</span>}
          </StyledInput>
        </div>
...
/**
 * One of the big benefits of using the React Context API is that you can break up your components
 * and have them consume the context without providing props to these components (resulting in "prop drilling").
 * E.g. You could create a separate component for the summaries, and have the following code
 *
 * const Summary = () => {
 *   const { totals } = useFormContext()
 *
 *   return (
 *   <div className="costSummaryAmounts">
 *    ...
...
/**
 * [LIBRARIES]
 * I see you're using className's a lot in this file.
 * This defeats the purpose of using styled components.
 *
 * Instead of having <div className="costSummaryAmounts">; rather create a `const SummaryAmountsWrapper = styled.div`, which
 * can style all it's children components.
 */

/**
 * [COMPONENTS]
 * Many of your inputs (text, dropdowns etc) can be atomicized for re-use. A lot of them look very similiar.
 * Use the rule of 3's here. If you're writing very similiar looking code at least 3 times, make it a shared component.
 */
